    ...
    for (int i = 0; i < current.size(); i++)
    {
        if (current[i] == '[')
        {
            ...
            if (nested == 5)
            {
                ...
                exploded = true;
                std::vector<std::string> currentSeparated = StringOperations::splitString(current.substr(i+1, current.find_first_of("]", i) - i - 1), ",");
...
int calculateResult(std::string current)
{
    ...
    while (current.find('[') != std::string::npos)
    {
        ...
        std::vector<std::string> currentSeparated = StringOperations::splitString(current.substr(iStart + 1, iEnd - 1), ",");
...
int main(void)
{
    ...
    std::cout << "Day 18, puzzle 1: " << getMagnitude(input) << std::endl;
    auto t_end = std::chrono::high_resolution_clock::now();
    std::cout << "Completed in: " << std::chrono::duration<double, std::milli>(t_end - t_begin).count() << " ms" << std::endl;

    assert(getLargestPossibleMagnitude(testInput2) == 3993);    
    t_begin = std::chrono::high_resolution_clock::now();    
    std::cout << "Day 18, puzzle 2: " << getLargestPossibleMagnitude(input) << std::endl;

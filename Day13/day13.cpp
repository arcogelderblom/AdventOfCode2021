...
std::set<Coordinate> toCoordinates(std::vector<std::string> stringCoordinates)
{
    ...
    {
        std::vector<std::string> splittedItem = StringOperations::splitString(item, ",");
...
int main(void)
{
    ...
    std::set<Coordinate> testInputDots = toCoordinates(StringOperations::splitString(StringOperations::splitString(testInput, "\n\n")[0], "\n"));
    std::vector<std::string> testInputInstructions = StringOperations::splitString(StringOperations::splitString(testInput, "\n\n")[1], "\n");
    assert(executeInstructions(testInputDots, testInputInstructions, 1).size() == 17);
    
    InputFile inputFile("InputFiles/day13.txt");
